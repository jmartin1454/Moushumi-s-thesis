...
stability~\cite{doe:website2}.  Allan deviations characterize changes
in the measured quantity when the data are averaged on different
timescales.  When the data behave statistically on short timescales,
the Allan deviation is equal to the standard deviation.  If drifts
occur, normally on longer timescales, the Allan deviation grows
linearly with a slope that $1/\sqrt{2}$ times the slope of the drift
in time.
...
shield.  The goal of subsequent work was twofold:
\begin{itemize}
\iteim to attempt to identify some of the sources of drift, especially
