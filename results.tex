...
\section{Application of different operational modes of the magnetometer}

\subsection{NMOR near zero field and degaussing studies}
...
to the $z$-coil with a 10000~$\Omega$ resistor in series which dominates
the resistance of the circuit.  Recall that the coil constant of the
$z$-coil is $\sim 48$~nT/mA (see Section~\ref{sec:Internal coil}).  The
sweeping range of this trace is therefore about
% (0.8~V)/(10000~Ohm)/*48~nT/mA = 3.84 nT ???
1.92 nT peak to peak.

In the first section of the oscilloscope trace, the impact of the
degaussing procedure on the optical rotation signal can be seen.  The
next section involves the ramping down of a variable resistor,
followed by opening the switch to isolate the degaussing coil.  Then
the magnetic field~($B_z$) is swept and the characteristic dispersive
Lorentzian shape of NMOR is observed.
...
\caption{Oscilloscope trace of measurement OR near zero field. The
  purple curve shows a voltage... that the magnetic field was ramped
  about 1.92 nT peak to peak with a triangular wave while the blue
  colored dispersive Lorentzian shape curve indicates another
  voltage... induced optical rotation due to field
  sweeping.\label{fig:TUNE} }
\end{figure}


In this measurement NMOR signal is used to determine effectiveness of degaussing procedure. After the degaussing procedure the observed field inside the shield is non-zero while the applied field $B_z$ was zero which might indicate the existence of some remnant field. Durig this measurement the magnetic field ($B_z$) was ramped about 2.5 nT peak to peak which corresponds to resonance width 0.4 nT. The effect of degaussing parameter on resonance width will describe on section \ref{sec:degaussing}.
\begin{table}[h]
\centering
\begin{tabular}{|l |l|}
\hline
