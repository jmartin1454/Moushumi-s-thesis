...
Section~\ref{sec:something-in-chapter-3-about-degaussing}.
...
\item The innermost magnetic shield was degaussed with various
  ...
  circuit was described in Section~\ref{sec:somewhere}.  After
...
\item The magnetic field along the $z$-direction (defined in
  Section~\ref{sec:somewhere-else}) is swept in order to calibrate the
...
to the $z$-coil with a 100~$\Omega$ resistor in series which dominates
the resistance of the circuit.  Recall that the coil constant of the
$z$-coil is $\sim 48$~nT/A (see Section~\ref{sec:zcoil}).  The
sweeping range of this trace is therefore about
% (800~mV)/(100~Ohm)/(1000 mA/A)*48~nT/A = 0.384 nT ???
2.5 nT peak to peak.
...
\caption{Oscilloscope trace of measurement OR near zero field. The
  ...
  about 2.5 nT peak to peak with a triangular wave while the blue
