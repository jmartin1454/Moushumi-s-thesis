...
field.  However, as shown in Ref.~\cite{bib:pustelny}, magnetometers
...
Section~\ref{sec:tilted-intro}.  One example is that it could give
...
Recall from Section~\ref{sec:tilted-intro}, in the work of
Ref.~\ref{cite:pustelny}, it was discovered that resonances in
...
Ref.~\cite{bib:pustelny}.  In addition to modulating the pump beam at
...
consistent with that measured by Ref.~\cite{bib:pustelny}.
...
Ref.~\cite{bib:pustelny}, to our knowledge they represent the first
measurement of a tilted field using FID mode.  An interesting aspect
not discussed in Ref.~\cite{bib:pustelny} is the persistence of the
